...
describe("MasonryRoot · scroll hysteresis render counts", () => {
    test("inert 16px wheel tick skips MasonryRoot commit and 0 ItemSlots rerender", () => {
        expect(filled.isWindowShiftInert(INERT_16.rest, INERT_16.shifted)).toBe(true);
    ...
    test("inert 64px also skips commit", () => {
        expect(filled.isWindowShiftInert(INERT_64.rest, INERT_64.shifted)).toBe(true);
    ...
    test("boundary crossing in col 0 forces MasonryRoot commit and exactly 1 ItemSlot change", () => {
        expect(filled.isWindowShiftInert(CROSSING_COL0.previous, CROSSING_COL0.next)).toBe(false);
    ...
    test("boundary crossing in last column forces commit and exactly 1 ItemSlot change (full scan)", () => {
        expect(filled.isWindowShiftInert(CROSSING_LAST.previous, CROSSING_LAST.next)).toBe(false);
...
describe("ItemSlot · memoization render counts", () => {
    test("arePlacementsEqual: identical placement is equal, moved item is not", () => {
        const a = filled.get(0)!;
        const b = { ...a };
        const c = { ...a, top: a.top + 1 };
        expect(arePlacementsEqual(a, b)).toBe(true);
        expect(arePlacementsEqual(a, c)).toBe(false);
        expect(arePlacementsEqual(null, null)).toBe(true);
        expect(arePlacementsEqual(a, null)).toBe(false);
        expect(arePlacementsEqual(null, a)).toBe(false);
    });
    ...
    test("areItemSlotPropsEqual: memo hits exactly when placement+identity equal", () => {
        ...
        expect(areItemSlotPropsEqual(base, { ...base, item: itemB })).toBe(true);
        // Misses: each is a distinct render cause
        expect(areItemSlotPropsEqual(base, { ...base, item: itemC })).toBe(false);
        expect(
        ...
        expect(areItemSlotPropsEqual({ ...base, item: null }, base)).toBe(false);
        expect(areItemSlotPropsEqual({ ...base, item: null }, { ...base, item: null })).toBe(true);
    });
    ...
    test("areItemSlotPropsEqual: summary hits/misses is deterministic", () => {
        ...
        const cases: [any, any, boolean][] = [
            [base, { ...base, item: { ...itemA } }, true],
            [base, { ...base, item: { ...itemA, top: itemA.top + 1 } }, false],
            ...
            [{ ...base, item: null }, base, false],
            [{ ...base, item: null }, { ...base, item: null }, true],
        ];
        let hits = 0,
            misses = 0;
...
describe("commitPendingMeasurements · batch dedup render counts", () => {
    test("duplicate index in same batch: last write wins, update reflows once per column", () => {
        ...
        expect(p.get(5)!.height).toBe(400);
        expect(p.get(20)!.height).toBe(250);
    });
    ...
    test("commitPendingMeasurements: unconnected or wrong-index nodes do not trigger renders", () => {
        ...
        expect(p.get(2)!.height).toBe(100);
    });
    ...
    test("commitPendingMeasurements: valid new index appends and valid update reflows", () => {
        ...
        expect(p.get(3)!.height).toBe(150);
        ...
        expect(p.get(1)!.height).toBe(220);
    });
});
