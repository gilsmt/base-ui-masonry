    ...
    <div className="space-y-5">
      ...
      <div className={styles.controls}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-500">Layout</h2>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" className={styles.button} onClick={() => loadMore()} disabled={isLoadingMore}>
          ...
          <button
            ...
            onClick={() => {
              setPins(createPins(INITIAL_COUNT, seed + Math.floor(Math.random() * 1000), 0));
              window.scrollTo(0, 0);
          ...
          <span className={styles.secondaryText}>
            Images: <code className={styles.kbd}>picsum.photos/seed</code> · Append is the fast path
            (stable <code className={styles.kbd}>key</code>); scroll to auto-load.
          </span>
        </div>
      </div>
      ...
      {scrollMode === 'contained' ? (
        ...
        >
          ...
          <Sentinel ref={sentinelRef} isLoading={isLoadingMore} onLoadMore={loadMore} hasMore={pins.length < 900} />
        </div>
      ) : (
        <>
...
const Sentinel = React.forwardRef<HTMLDivElement, { isLoading: boolean; onLoadMore: () => void; hasMore: boolean }>(
      ...
      <div ref={ref} className="flex flex-col items-center gap-3 px-4 py-8">
        {hasMore ? (
          <>
            ...
            <span className="text-xs text-gray-500">Or scroll — next batch loads automatically.</span>
          </>
        ) : (
